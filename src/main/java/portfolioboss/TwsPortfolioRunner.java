...
import portfolioboss.api.SnapshotStore;
...
/**
 * The startup flow: once the web server is up, reads the portfolio from TWS, hands it to the API
 * and opens the UI. It is the only place that connects to TWS, and it owns the TWS host, port and
 * client id. {@code ./run.sh 7497 102} arrives as the two non-option arguments (port, client id).
 *
...
class TwsPortfolioRunner implements ApplicationRunner {
    ...
    TwsPortfolioRunner(SnapshotStore snapshotStore,
    ...
    public void run(ApplicationArguments arguments) throws InterruptedException {
        ...
        if (snapshot == null) {
            // Nothing to serve. Closing the Spring context stops the web server; the TWS reader
            // loop runs on a daemon thread, so the JVM exits right after.
            System.exit(SpringApplication.exit(context));
        }

        snapshotStore.set(snapshot);
        System.out.println("[api] serving the snapshot at http://localhost:" + apiPort + "/api/portfolio");
