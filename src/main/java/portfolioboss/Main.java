 ...
 * and average cost come straight from the broker), prints them, disconnects, stores them in the
 * PostgreSQL database, and then serves them from there to the UI on localhost
 * ({@code GET /api/portfolio}) until stopped, starting the UI and opening it in the browser. This proves the one IB call the long-term portfolio platform is built
